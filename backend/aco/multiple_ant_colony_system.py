...
class MultipleAntColonySystem:
        ...
        while True:
            ...
            if logger and (time.time() - last_log_time) >= log_every_seconds:
                best_dist = (
                    ...
                    if global_best_distance is not None
                    else ant_best_travel_distance
                )
                best_dist = 0.0 if best_dist is None else best_dist
                logger(
                    "ACO acs_time iter %d: best_cost=%.2f, vehicles=%d"
                    % (iteration, best_dist, vehicle_num)
                )
                last_log_time = time.time()
        ...
        while True:
            ...
            if logger and (time.time() - last_log_time) >= log_every_seconds:
                best_dist = (
                    global_best_distance
                    if global_best_distance is not None
                    else current_path_distance
                )
                best_dist = 0.0 if best_dist is None else best_dist
                logger(
                    "ACO acs_vehicle iter %d: best_cost=%.2f, vehicles=%d"
                    % (iteration, best_dist, vehicle_num)
                )
                last_log_time = time.time()
    ...
    def run_multiple_ant_colony_system(
        self,
        ...
        if self.whether_or_not_to_show_figure:
            ...
            self._multiple_ant_colony_system(
                path_queue_for_figure,
                file_to_write_path,
                logger,
                log_every_seconds,
    ...
    def _multiple_ant_colony_system(
        ...
        while True:
            ...
            if logger and (time.time() - last_log_time) >= log_every_seconds:
                ...
                logger(
                    "ACO iter %d: best_cost=%.2f, vehicles=%s, elapsed=%.1fs"
                    % (
                        iteration,
                        self.best_path_distance.value,
                        self.best_vehicle_num,
