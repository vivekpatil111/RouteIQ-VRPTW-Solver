...
class Solver:
    ...
    def print_solution(self):
        ...
        """
        assert self.manager is not None and self.routing is not None and self.solution is not None
    ...
    def get_solution_time(self):
        ...
        """
        assert self.routing is not None and self.solution is not None
        if self.routing.status() == 1:
            time_dimension = self.routing.GetDimensionOrDie("Time")
            total_time = 0
            for vehicle_id in range(self.data.num_vehicles):
